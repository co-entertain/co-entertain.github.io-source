...
def encrypt_content(content):
    ...
    """
    ciphertext_bundle = encrypt_text_aes(content._content, settings['password'])
...
def pelican_content_in_categaries(content_generators):
    """
    Finds pages and articles in specific categories and processes them.
    """
    for generator in content_generators:
        if isinstance(generator, generators.ArticlesGenerator):
            for article in generator.articles + generator.translations:
                ca =  getattr(article, 'category')
                if ca.name in settings['categories']:
                    # print ca.name
                    encrypt_content(article)
...
def register():
    ...
    signals.all_generators_finalized.connect(pelican_content_in_categaries)
